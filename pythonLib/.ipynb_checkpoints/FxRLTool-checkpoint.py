...
class FxDealerTrainer(gym.Env):
    ...
    def __init__(self, diff = True, nbNotUsedTablediff = True,\
                ...
                std_sep = StdNormalizer.STD_SEP, stdX = [],\
                 used_index_n = 4, isForMakingTeacherData = False, isPure = False):
        ...
        #self.isForMakingTeacherData 教師あり学習に使うとき用
        #self.beforeAct 前回の売り買いがどっちか
        #self.sameActTime 何回同じ向きの売り買いをしたか
        #self.buyTime  何回買ったか
        #self.sellTime 何回売ったか
        ...
        if(acc != 0):#何か行動したら
            ...
            if(self.Holding != 0):# 建玉持ってたら
                if(acc == 2):
                    ...
                    if(self.sameActTime > 1 and reward > 0):
                        reward /= self.sameActTime
                ...
                if(acc != 2):
                    ...
                    if(self.beforeAct == -1 * acc):
#                        reward += 0.008
                        self.sameActTime = 0
                    if(acc == 1):
                        self.buyTime += 1
                        reward += 0.008 * self.sellTime / (self.sellTime + self.buyTime)
                    else:
                        self.sellTime += 1
                        reward += 0.008 * self.buyTime / (self.sellTime + self.buyTime)
...
#        else:
#            if(self.Holding * (self.nowClose - self.startHoldingValue) > 0):
#                reward += 0.2 * 0.5/ (self.countTime ** (1/2)) 
...
class FxEstimater(gym.Env):
    SPREAD = 0.006
    
    def __init__(self, diff = True, type = "updown1", x_window = 100, y_window = 20,\
                 YBinLimit = 0.14, nbNotUsedTablediff = True,\
                 prepareWindow = 20, fileGlob = "./minute/*.csv" ,\
                std_sep = StdNormalizer.STD_SEP, stdX = [],\
                 used_index_n = 4):
        #self.X        値動きの表　差分をとり正規化済　
        #self.Y        予測ラベル　ワンホットに加工済み
        #self.Xnorm      self.Xを正規化する
        #self.nbList    一続きのself.Xの数　len(self.X), len(self.closeList)に対応 
        #self.stateSize  1stepの状態空間の大きさ　＝　self.X[i][i, :, :]の大きさ
        #self.y_window 何分後の値動きを予想するか
        #self.x_window 何分間のデータを使うか
        ...
        self.action_space = gym.spaces.Discrete(4) # 行動空間。下がる、普通　上がる　保留　の４つ
        fileList = glob.glob(fileGlob)
        self.dataLoader = FxDataLoader(fileList)
        self.X = self.dataLoader.get_X(1, y_window, diff = diff, isForRL = True)
        self.Xnorm = StdNormalizer(self.X[0], std_sep, False, stdX)
        self.X = [self.Xnorm.stdNormalize(i)[:, :, :used_index_n] for i in self.X]
        self.Y = self.dataLoader.get_Y(1, y_window, type = type)
        self.Y = self.dataLoader.get_Y_binary(self.Y, YBinLimit)
...
#         self.reward_range = [-1, 1]
    ...
    def step(self, action):
        reward = 0
        print( self.nowY.shape)
        trueAns = self.nowY[self.stepIndex]
        print(type(trueAns))

        if(self.stepIndex >= self.x_window):
            if(action == 3):
                reward += 0.1
            elif(action == trueAns):
                reward += 0.3
    ...
    def resetDefineIndex(self, episodeIndex):
        ...
        self.stepIndex = 0
        self.nowX = self.X[self.episodeIndex]
        self.nowY = self.Y[self.episodeIndex]
        return self.getNowState()
